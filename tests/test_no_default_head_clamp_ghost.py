...
def test_production_yaml_disables_crop_stitch_head_clamp():
    ...
    assert cfg.get("stitch_mask_dilate_px", 4) == 12
    assert cfg.get("head_matte_expand_px", 3) == 6
    ...
    assert int(cfg.get("neck_stub_tone_match_ring_px", 40)) == 90
    assert cfg.get("collapse_soft_chin_ghost") is True
    assert float(cfg.get("donor_scale_factor", 1.0)) == 0.78
